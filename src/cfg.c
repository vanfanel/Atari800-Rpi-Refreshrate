	...
	while (fgets(string, sizeof(string), fp)) {
		...
		if (ptr != NULL) {
			*ptr++ = '\0';
			...
			else if (strcmp(string, "MONITOR_REFRESH_RATE_PAL") == 0) {
				double num;
				Util_sscandouble(ptr, &num);
				Atari800_PhysicalRefreshRatePAL = num;
			}	
			else if (strcmp(string, "MONITOR_REFRESH_RATE_NTSC") == 0) {
				double num;
				Util_sscandouble(ptr, &num);
				Atari800_PhysicalRefreshRateNTSC = num;
			}
...
int CFG_WriteConfig(void)
{
	...
	fprintf(fp, "MONITOR_REFRESH_RATE_PAL=%f\n", Atari800_PhysicalRefreshRatePAL);
	fprintf(fp, "MONITOR_REFRESH_RATE_NTSC=%f\n", Atari800_PhysicalRefreshRateNTSC);
